...
@with_kw mutable struct mso_params_ops1{F<:Function}
    ...
    Q::Matrix{Float64} = zeros(6,6)           # Covariance AbstractMatrix for unknown (random) forcing
    Q_inv::Float64
    R::Matrix{Float64} = zeros(6,6)           # Covariance AbstractMatrix for noise in data
    R_inv::Matrix{Float64}                    # Inverse of operator R

    K::Matrix{Float64} = zeros(6,6)           # Placeholder for Kalman gain AbstractMatrix 
    Kc::Matrix{Float64} = zeros(6,6)     

end

function mso_params_ops1(;
    T::Int,               # Total steps to integrate 
    t::Int = 0,           # placeholder for current step
    dt::Float64 = 0.001,  # timestep
    ...
    Kc::Matrix{Float64} = zeros(6,6),
        ...
        mso_params_ops1(T, t, dt, x, u, n, k, r, q, J, data_steps, data, states, energy, A, B, Gamma, E, Q, Q_inv, R, R_inv, K, Kc)
...
function mso_params1(; 
    ...
    energy::Matrix{Float64},    # placeholder for computed energy
        ...
        mso_params1(T, dt, x, u, n, k, r, q, J, data_steps, data, states, energy)
end
...
function mso_operators1(; 
    ...
    Kc::Matrix{Float64} = zeros(6,6),
        ...
        mso_operators1(A, B, Gamma, P0, P, E, Q, R, K, Kc)
end
...
function cost_function_eval(k_guess)
    ...
    R = cov(params_true.n[:], corrected=false)
    R = R * diagm(ones(6))
    ...
    params_true.states[:,1] = copy(params_true.x)
    ...
    for j = 2:T+1
        ...
        params_true.states[:, j] = copy(params_true.x)

        states_noisy[:, j] = copy(params_true.x + params_true.n[:, j])
        temp += params_true.dt
...
function gradient_eval(G, k_guess)
    ...
    autodiff(Reverse, integrate1, Duplicated(params_adjoint, dparams_adjoint))

    G[1] = dparams_adjoint.k

    return nothing

end

# @code_warntype cost_function_eval([20.], 0.0)

# k0 = [31.]
# optf = OptimizationFunction(cost_function_eval, Optimization.AutoEnzyme())
# prob = OptimizationProblem(optf, k0)
# @code_warntype solve(prob, BFGS())

# sol = optimize(cost_function_eval, gradient_eval, [29.], Optim.options(allow_f_increases = true))
