...
function run_ensemble_kf(N, Ndays)
    ...
    # Assume incorrect initial conditions for the KF
    ...
    # Output will be stored in the matrix Z
    Z = zeros(48896, N)
    for i = 1:N
        ...
        for t = 1:S_kf.grid.nt
            ...
            Z[:, i] = [u_mat_to_vec(temp.u); v_mat_to_vec(temp.v); eta_mat_to_vec(temp.η)]
    ...
    for t = 1:S_kf.grid.nt
        ...
        if t ∈ data_steps
            ...
            Y = u_mat_to_vec(temp.u) * Π

            S .= Y
            D̃ = D - u_mat_to_vec(temp.u)
            W = S' * (S*S' + E*E')^(-1)*D̃
