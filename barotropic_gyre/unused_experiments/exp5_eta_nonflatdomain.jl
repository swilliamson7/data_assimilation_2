...
function exp5_cpintegrate(chkp, scheme)::Float64
    ...
    @ad_checkpoint scheme for chkp.i = 1:chkp.S.grid.nt
        ...
        if i in chkp.data_steps
            ...
            chkp.J += sum((vec(temp.η) - chkp.data[:, chkp.j]).^2 ./ 128^2)
    ...
    chkp.J += sum((chkp.pred_avg_eta .- chkp.avg_eta).^2 ./ 128^2) / chkp.S.parameters.Ndays
...
function compute_initcond_newoptimizer()
    ...
    # ekf_avgu, ekf_avgv, ekf_avgeta = run_ensemble_kf(ekf_model, param_guess; udata=false, vdata=false, etadata=true)
    ...
    return ekf_avgu, ekf_avgv, ekf_avgeta, data, true_states, result, S_adj, states_adj, S_pred, pred_states
