mutable struct exp4_adj_model{T, S} <: AbstractNLPModel{T,S}
...
mutable struct exp4_ekf_model{T}
    S::ShallowWaters.ModelSetup{T,T}        # model struct for adjoint
...
function exp4_model_setup(T, Ndays, N, sigma_data, sigma_initcond, data_steps, data_spots)
    ...
    adj_model = exp4_adj_model{T, typeof(param_guess)}(meta,
        Counters(),
        Sadj,
        data,
        data_steps,
...
function NLPModels.obj(model, param_guess)
    ...
    model.S.parameters.Fx0 = param_guess[1]

    return exp4_integrate(model)
...
function run_exp4()
    ...
    adj_model, ekf_model, param_guess, S_pred, P_pred, true_states = exp4_model_setup(Float64, Ndays,N,sigma_data,sigma_initcond,data_steps,data_spots)
    ...
    S_adj.parameters.Fx0 = result.solution[1]

    _, states_adj = exp4_generate_data(S_adj, data_steps, data_spots, sigma_data)
